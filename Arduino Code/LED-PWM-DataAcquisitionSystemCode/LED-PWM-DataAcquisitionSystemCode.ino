...
void loop() {
  ...
  for (int intensity = 0; intensity <= 255; intensity++) {
    analogWrite(LEDPin, intensity);  // Set PWM duty cycle for speed control
    delay(10);  // Delay for smooth speed transition
    if (Serial.available()){
  ...
  // Decrease motor speed gradually from maximum to 0
  ...
  if (myFile) // it opened OK
    {
      ...
      myFile.print("\tgyroZ : ");myFile.println(mpu6050.getGyroZ());
      myFile.print("Brightness : ");myFile.println(brightness);
      Serial.println("=======================================================");
